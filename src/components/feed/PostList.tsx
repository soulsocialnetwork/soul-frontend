    ...
    return (
      <div className="pt-3 grid grid-cols-2 gap-4 px-4">
        {Array.from({ length: 4 }).map((_, i) => (
          <div
            key={i}
            className="aspect-square bg-surfaceHighlight rounded-2xl animate-pulse"
          />
        ))}
      </div>
    );
  ...
  return (
    <div className="pt-2 pb-8 flex flex-col min-h-[70vh]">
      ...
      {currentPage >= totalPages - 1 && posts.length > ITEMS_PER_PAGE && (
        <div className="mt-8 mx-4 p-5 rounded-2xl bg-surface/30 border border-white/5 flex flex-col items-center justify-center text-center backdrop-blur-sm animate-fade-in">
          <h3 className="text-[14px] font-semibold text-textPrimary mb-1">
            Você viu tudo
          </h3>
      ...
      {selectedPost && (
        ...
        >
          ...
          <div
            className="flex-1 flex items-center py-10"
